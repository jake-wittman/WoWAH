...
updateItemDB <- function(item.db.path = NULL, item.id) {
   ...
   new_db <- map_dfr(id_chunks, function(.x) {
      ...
      # data.id column is the item id
      item_df <- item_list$results %>% 
         select(-contains(c("GB", "ES", "DE", "RU", "KR", "BR", "FR", "TW", "IT", "CN", "MX"), ignore.case = FALSE))
                
      item_df <- item_df %>% 
         select(data.level, data.sell_price, data.purchase_price, data.id,
                data.item_subclass.name.en_US, data.item_class.name.en_US,
                data.quality.name.en_US, data.name.en_US)
   })
...
updateItemDB(item.db.path = here::here("data", "item_db.csv"),
             item.id = unique(AH_df$item_id))
